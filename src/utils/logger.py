...
def _resolve_level(level: int | str | None) -> int:
    ...
    if isinstance(level, str):
        # getLevelName() returns the string "Level FOO" for unknown names instead of raising,
        # which only blows up later inside setLevel(). Fall back to INFO on a typo.
...
def get_logger(name: str = "moodnote") -> logging.Logger:
    """
    Get a child logger under the "moodnote" root.

    Deliberately does not attach handlers: modules import this at module scope, and
    configuring here would create a log file on every import. Entry points call
    `setup_logger()` once; these loggers propagate up to it.

    Args:
        name: Logger name, relative to the "moodnote" root
    ...
    return logging.getLogger(f"moodnote.{name}")
